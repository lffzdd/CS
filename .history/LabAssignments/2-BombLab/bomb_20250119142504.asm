...
0000000000400ee0 <phase_1>:
  ...
  400ef0:	74 05                	je     400ef7 <phase_1+0x17> ; 如果ret0说明rdi和rsi相等,否则就会爆炸,因此rdi为输入字符串,rsi为0x402400
...
000000000040131b <string_length>: ; rdi 指向字符串起始地址
  40131b:	80 3f 00             	cmpb   $0x0,(%rdi)
  40131e:	74 12                	je     401332 <string_length+0x17> ;如果字符串开为0,表示eof,那么就直接返回长度为0
  401320:	48 89 fa             	mov    %rdi,%rdx
  401323:	48 83 c2 01          	add    $0x1,%rdx  ; rdx 第一次指向字符串开头+1 ; 后面慢慢的往后移动
  401327:	89 d0                	mov    %edx,%eax  ; rax 存储了当前移动到的位置
  401329:	29 f8                	sub    %edi,%eax  ; 移动到的位置减去起始位置再+1,也就是字符串长度
  40132b:	80 3a 00             	cmpb   $0x0,(%rdx); 字符串的地址+1是否为eof,不是就跳回 add    $0x1,%rdx
  40132e:	75 f3                 	jne    401323 <string_length+0x8>
  401330:	f3 c3                	repz ret ;repz 是 x86 汇编语言中的一个前缀指令，它的全称是 "repeat while zero"。这个前缀通常与字符串操作指令（如 movsb、stosb 等）一起使用，用于在条件满足时重复执行这些指令。在你的代码片段中，repz ret 实际上是一个优化的返回指令。repz 前缀在这里没有实际的作用，因为 ret 指令本身不会重复执行。这个组合通常用于填充指令长度，以优化指令缓存对齐。
  401332:	b8 00 00 00 00       	mov    $0x0,%eax
  401337:	c3                   	ret

0000000000401338 <strings_not_equal>: ; 有rdi和rsi两个字符串作为参数,ret0说明相等,1则不相等
  401338:	41 54                	push   %r12
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  40133c:	48 89 fb             	mov    %rdi,%rbx
  40133f:	48 89 f5             	mov    %rsi,%rbp
  401342:	e8 d4 ff ff ff       	call   40131b <string_length> ; 该函数只有一个参数rdi,存储字符串起始地址,返回字符串长度存储在rax中
  401347:	41 89 c4             	mov    %eax,%r12d ; 把字符串长度存储在r12d中
  40134a:	48 89 ef             	mov    %rbp,%rdi  ; 把rdi作为字符串起始地址,则rsi也是存储字符串地址
  40134d:	e8 c9 ff ff ff       	call   40131b <string_length>
  401352:	ba 01 00 00 00       	mov    $0x1,%edx  ; rdx 存储1
  401357:	41 39 c4             	cmp    %eax,%r12d ; 把rsi字符串和rdi字符串长度作比较
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63> ; 长度不相等就ret,值为1
  40135c:	0f b6 03             	movzbl (%rbx),%eax  ;rbx存储的是rdi,也就是rax存储rdi字符串第一个字符
  40135f:	84 c0                	test   %al,%al ;如果字符串起始为eof,则ret,值为0,这里说明两者都是空字符串
  401361:	74 25                	je     401388 <strings_not_equal+0x50>
  401363:	3a 45 00             	cmp    0x0(%rbp),%al  ;rbp存储的是rsi,比较rdi和rsi第一个字符
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a>  ;若相等则进入循环
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57>  ;不相等则ret1
  40136a:	3a 45 00             	cmp    0x0(%rbp),%al
  40136d:	0f 1f 00             	nopl   (%rax) 
  401370:	75 24                	jne    401396 <strings_not_equal+0x5e> ; 不相等则ret1
  401372:	48 83 c3 01          	add    $0x1,%rbx  ; *s1+1
  401376:	48 83 c5 01          	add    $0x1,%rbp  ; *s2+1
  40137a:	0f b6 03             	movzbl (%rbx),%eax
  40137d:	84 c0                	test   %al,%al    ; if *s1+1!=0x0
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32>
  401381:	ba 00 00 00 00       	mov    $0x0,%edx  ; 若直到s1结束都相等,则返回0
