...
ssize_t rio_readlineb(rio_t* rp, void* usrbuf, size_t maxlen) {
  ...
  for (n = 1; n < maxlen; n++) {
    if ((rc = rio_read(rp, &c, 1)) == 1) { // rio_read 把rp->rio_fd中的数据读到rp->rio_buf中,再把rp->rio_buf中的数据读到usrbuf中,每次读一个字节
      *bufp++ = c; // 1.将 c 的值赋给 *bufp，即 bufp 当前指向的内存位置。2.将 bufp 指针向前移动一个位置。
      if (c == '\n') { // \n 已经添加到了userbuf中
        n++;
        break;
      }
    }
    else if (rc == 0) { // 如果rc没有读到数据,则说明读到了EOF
      if (n == 1)
